...
public class EnvioController {
    ...
    String sql = """
        INSERT INTO ENVIO (
            ...
            direccion_destino, tipo_envio, estado_actual, fecha_entrega_estimada, 
            costo, descuento_aplicado, costo_final, tienda_origen_id, tienda_destino_id, 
    ...
    try (Connection cn = Conexion.conectar();
        ...
        if (e.getFechaEntregaEstimada() != null) {
            ps.setTimestamp(8, new Timestamp(e.getFechaEntregaEstimada().getTime()));
        } else {
            ps.setNull(8, java.sql.Types.TIMESTAMP);
        }

        ps.setDouble(9, e.getCosto());
        ps.setDouble(10, e.getDescuentoAplicado());
        ps.setDouble(11, e.getCostoFinal());
        
        // tienda_origen_id
        if (e.getTiendaOrigenId() != null) {
            ps.setLong(12, e.getTiendaOrigenId());
        } else {
            ps.setNull(12, java.sql.Types.BIGINT);
        }

        // tienda_destino_id
        if (e.getTiendaDestinoId() != null) {
            ps.setLong(13, e.getTiendaDestinoId());
        } else {
    ...
    try (Connection cn = Conexion.conectar();
        ...
        if (rs.next()) {
            long total = rs.getLong(1) + 1;
            return String.format("ENV%06d", total);
        }
